...
# This provides services on determining how many moves from solved a state is
from movelibrary import MoveLibrary

from modenormal import ModeNormal
from modeMatchThree import ModeMatchThree
from modeSymmetricSolve import ModeSymmetricSolve
from modePartialSolve import ModePartialSolve
...
class GrooviksCube:
	...
	def __init__(self, logger, display):
		...
		self.__tetrisState = StateTetris()
		...
		self.__matchMode = ModeMatchThree()
		self.__symmetricMode = ModeSymmetricSolve()
		self.__partialMode = ModePartialSolve()
	...
	def __ProcessQueuedStates( self, currentTime ):
		...
		if ( self.__currentCubeState == CubeState.ROTATING ):
			...
			self.__currentState = self.__fireState
		elif ( self.__currentCubeState == CubeState.TETRISMODE ):
			self.__currentState = self.__tetrisState
	...
	def __SwitchMode( self, params ):
		...
		elif(params[0] == CubeMode.MATCH3):
			self.__currentMode = self.__matchMode
			self.LogEvent( "Switching to Match3 Game mode" )
		elif(params[0] == CubeMode.SYMMETRIC):
			self.__currentMode = self.__symmetricMode
			self.LogEvent( "Switching to Symmetric Game mode" )
		elif(params[0] == CubeMode.PARTIAL):
			self.__currentMode = self.__partialMode
			self.LogEvent( "Switching to Partial Game mode" )
